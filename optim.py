...
for i in range(len(test_images)):
                image=test_imagesNoise[i]
                image = random_noise(image, mode='s&p', amount=.01)
...
# test_imagesNoise=np.copy(test_images)
# for i in range(len(test_images)):
#                 gauss = np.random.normal(0,0.2,(32,32,3))
#                 gauss = gauss.reshape(32,32,3)
#                 image=(test_imagesNoise[i]+gauss)
#                 image=np.clip(image, 0, 1)
#                 test_imagesNoise[i]=image
...
plt.title('Speckle Noise in Image and .1 STD Noise in Test Network')
...
plt.xlabel('Noise in Image (mean*100)')
